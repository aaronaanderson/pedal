...
class Window {
  ...
  static inline float hanningFromPhase(float inputPhase){
    return 0.5f * (1.0f - std::cos(2.0f * pdlPI * inputPhase));
  }
  static inline float hammingFromPhase(float inputPhase){
    return 0.54 - (0.46 * std::cos(2.0f * pdlPI * inputPhase));
  }
  static inline float cosineFromPhase(float inputPhase){
    return std::sin(pdlPI * inputPhase);
  }
  ...
  static inline float blackmanNutallFromPhase(float inputPhase){
    return 0.3635819f - 
       (0.4891775f * std::cos(2.0f * pdlPI * inputPhase)) + 
       (0.1365995f * std::cos(4.0f * pdlPI * inputPhase)) + 
       (0.0106411f * std::cos(6.0f * pdlPI * inputPhase));
  }
