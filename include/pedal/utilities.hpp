...
class SmoothValue{
  ...
  void calculateCoefficients(){//called only when 'setTime' is called (and in constructor)
    a = std::exp(-(pedal::PI * 2) / (arrivalTime * 0.001f * pdlSettings::sampleRate));//rearranged lpf coeff calculations
    b = 1.0f - a;
  }
...
public:
  ...
  void calculateUpCoefficients(){//called only when 'setTimeUp' is called (and in constructor)
    if(arrivalTimeUp > 0.0f){//avoid / 0
      aUp = std::exp(-(pedal::PI * 2.0f) / (arrivalTimeUp * 0.001f * pdlSettings::sampleRate));//rearranged lpf coeff calculations
      bUp = 1.0f - aUp;
    }else{
      aUp = 0.0f;
      bUp = 1.0f;
  ...
  void calculateDownCoefficients(){//called only when 'setTimeUp' is called (and in constructor)
    if(arrivalTimeDown > 0.0f){//avoid / 0
      aDown = std::exp(-(pedal::PI * 2.0f) / (arrivalTimeDown * 0.001f * pdlSettings::sampleRate));
      bDown = 1.0f - aDown;
    }else{
      aDown = 0.0f;
      bDown = 1.0f;
